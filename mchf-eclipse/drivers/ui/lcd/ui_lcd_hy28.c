...
{
    mchf_display.DrawLine(x0,y0,x1,y1,color);
}
...
void UiLcdHy28_DrawLine_ILI(int16_t x0, int16_t y0, int16_t x1, int16_t y1, uint16_t color) {
  ...
  for (; x0 <= x1; x0++) {
    if (steep) {
      UiLcdHy28_DrawColorPoint_ILI(y0, x0, color);
    } else {
      UiLcdHy28_DrawColorPoint_ILI(x0, y0, color);
    }
    err -= dy;
    if (err < 0) {
      y0 += ystep;
...
{
    mchf_display.FillTriangle(x0,y0,x1,y1,x2,y2,color);
}
...
//    writeFastHLine(a, y0, b - a + 1, color);
    mchf_display.DrawStraightLine(a ,y0, b - a  + 1, LCD_DIR_HORIZONTAL, color);
//    endWrite();
    return;
  }
...
const uhsdr_display_info_t display_infos[] = {
...
#ifdef USE_GFX_ILI932x
        {
				...
				.DrawStraightLine = UiLcdHy28_DrawStraightLine_ILI,
				.DrawFullRect = UiLcdHy28_DrawFullRect_ILI,
				.FillTriangle = UiLcdHy28_FillTriangle_ILI,
				.DrawColorPoint = UiLcdHy28_DrawColorPoint_ILI,
        },
#ifdef USE_GFX_SSD1289
        {
